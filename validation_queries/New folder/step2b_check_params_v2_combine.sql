...
from (Select  Distinct
    ...
    , substring(planner_cell.name, 1,
            IF(LOCATE('-', planner_cell.name) =0,
                100, LOCATE('-', planner_cell.name))-2) as trimmed_cell_name
        ...
        from (
            ...
            from (
             select distinct
                ...
                substring(planner_cell.name, 1,
                    IF(LOCATE('-', planner_cell.name) =0,
                    100, LOCATE('-', planner_cell.name))-2) as 'last_trimmed_cell_name',
            ...
            where
             substring(planner_cell.name, 1,
                IF(LOCATE('-', planner_cell.name) =0,
                100, LOCATE('-', planner_cell.name))-2)
             in (
                select distinct
                    trimmed_cell_name
                from (Select  Distinct
